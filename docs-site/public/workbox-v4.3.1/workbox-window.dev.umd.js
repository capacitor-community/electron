  ...
  */
  ...
  var EventTargetShim =
    ...
    (function () {
      ...
      _proto._getEventListenersByType = function _getEventListenersByType(
        type
      ) {
  ...
  var Workbox =
    ...
    (function (_EventTargetShim) {
      ...
      function Workbox(scriptURL, registerOptions) {
        ...
        _this._onMessage = _this._onMessage.bind(
          _assertThisInitialized(_assertThisInitialized(_this))
        );
        _this._onStateChange = _this._onStateChange.bind(
          _assertThisInitialized(_assertThisInitialized(_this))
        );
        _this._onUpdateFound = _this._onUpdateFound.bind(
          _assertThisInitialized(_assertThisInitialized(_this))
        );
        _this._onControllerChange = _this._onControllerChange.bind(
          _assertThisInitialized(_assertThisInitialized(_this))
        );
        return _this;
      }
      ...
      _proto.register = _async(function (_temp) {
        ...
        return _invoke(
          ...
          function () {
            ...
            return _await(_this2._registerScript(), function (
              ...
              if (_this2._compatibleControllingSW) {
                ...
                _this2._controllingDeferred.resolve(
                  _this2._compatibleControllingSW
                );

                _this2._reportWindowReady(_this2._compatibleControllingSW);

                _this2._compatibleControllingSW.addEventListener(
                  "statechange",
                  _this2._onStateChange,
                  {
                    once: true,
                  }
                );
              ...
              if (
                waitingSW &&
                urlsMatch(waitingSW.scriptURL, _this2._scriptURL)
              ) {
...

              {
                ...
                var currentPageIsOutOfScope = function currentPageIsOutOfScope() {
                  var scopeURL = new URL(
                    _this2._registerOptions.scope || _this2._scriptURL,
                    document.baseURI
                  );
              ...
              _this2._registration.addEventListener(
                "updatefound",
                _this2._onUpdateFound
              );

              navigator.serviceWorker.addEventListener(
                "controllerchange",
                _this2._onControllerChange,
                {
                  once: true,
                }
              ...
              if ("BroadcastChannel" in self) {
                ...
                _this2._broadcastChannel.addEventListener(
                  "message",
                  _this2._onMessage
                );
              }
      ...
      _proto._onUpdateFound = function _onUpdateFound() {
        ...
        var updateLikelyTriggeredExternally = // Since we enforce only calling `register()` once, and since we don't
          ...
          performance.now() >
            this._registrationTime + REGISTRATION_TIMEOUT_DURATION // If any of the above are not true, we assume the update was
